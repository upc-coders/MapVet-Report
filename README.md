# Universidad Peruana de Ciencias Aplicadas
## Ingeniería de Software
### Aplicaciones Web - Trabajo Final

</center>

<p align = "center"> <img src = "https://upload.wikimedia.org/wikipedia/commons/f/fc/UPC_logo_transparente.png"></img> </p>

<center>

- **Sección:** SW51
- **Docente:** Juan Carlos Tinoco Licas
- **Ciclo:** 2023-02
- **Nombre de la Startup:** upc-coders
- **Nombre del producto:** VetCare

## Integrantes
| Alumno  | Código    |
|----------|----------|
| Renato German Reyes Valenzuela   | U20221B471 |
| Juan Jesús Calisaya Sánchez       | U202121935 |
| Daniel Valverde Lopez  		   | U202121440 |
| Sebastian Andre Ramirez Mendez   | U20191E575 |
| Juliana Yauricasa Seguil		   | U20201b782 |

### Septiembre 2023


------------------------------------------------------
# Registros de versiones del Informe

| Version |  Fecha   | Autor                                                                                      | Descripción de la modificación                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
|:-------:|:--------:|:-------------------------------------------------------------------------------------------|:------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
|    1    | 02/09/23 | Juan Calisaya<br>Sebastian Ramirez<br>Renato Reyes<br>Daniel Valverde<br>Juliana Yauricasa | Culminación de los siguientes capítulos:</br></br>Capítulo I: Introducción</br>Se presenta el proyecto, comenzando con una descripción detallada de la startup y sus miembros fundadores. Se exploran los desafíos y problemas actuales que enfrenta la startup, empleando un enfoque de Lean UX para identificar problemas, Assumptions e Hypothesis Statements . Además, se delinean los segmentos objetivo para la solución propuesta.</br></br>Capítulo II: As-Is Software Solution Requirements Elicitation & Analysis</br>Se realiza un análisis del entorno competitivo de la startup, investigando estrategias y tácticas frente a los competidores. Se llevan a cabo entrevistas con el fin de obtener información esencial de los usuarios. Varias técnicas, como el diseño de entrevistas, el registro y análisis de entrevistas, y la identificación de necesidades a través de la creación de User Personas, User Task Matrix, User Journey Mapping y Empathy Mapping, son empleadas para este propósito.</br></br>Capítulo III: As-Is Software Solution Requirements Specification</br>Se especifican los requisitos del software. Se crean los To-Be Scenario Mapping y las User Stories. Además, se utiliza el Impact Mapping para visualizar cómo las funcionalidades afectan a los objetivos del proyecto y se establece un Product Backlog.</br></br>Capítulo IV: Product Design</br>Se diseña el producto, incluyendo pautas de estilo, arquitectura de la información, diseño de la página de aterrizaje, UX/UI de aplicaciones web, prototipado, arquitectura de software, diseño orientado a objetos y diseño de base de datos.</br></br>Capítulo V: Product Implementation, Validation & Deployment</br>Se desarrolla la implementación, validación y despliegue del producto, incluyendo gestión de configuración, implementación de páginas y aplicaciones, validación a través de entrevistas y pruebas, y finalmente, el despliegue del software. |
|    2    | 29/09/23 | Juan Calisaya<br>Sebastian Ramirez<br>Renato Reyes<br>Daniel Valverde<br>Juliana Yauricasa | Mejora continua - Corrección de los siguientes capítulos: </br></br>Capítulo II: As-Is Software Solution Requirements Elicitation & Analysis</br>Corrección del formato de las entrevistas y resúmenes. <br><br>  Capítulo III: As-Is Software Solution Requirements </br> Agregar user stories para la landing page y las technical stories, priorizar las user stories de acuerdo al core del negocio.</br><br>Capítulo IV: Product Design</br>Se corrigió el apartado UX/UI de la aplicación web y el prototipo.<br><br/>Capitulo V: Se agregaron todos los componentes faltantes del sprint 1, siendo un trabajo colaborativo del equipo.<br/>Creación de la versión 1 del frontend del sitio web utilizando Vue. <br/><br/>Corrección de la landing page.<br/>Creación del Student Outcome y corrección del cuadro de registro de versiones.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
|    3    | 04/11/23 | Juan Calisaya<br>Sebastian Ramirez<br>Daniel Valverde<br>Juliana Yauricasa                 | Mejora continua - Corrección de los siguientes capítulos: <br><br>  Capítulo III: Agregar user technical stories para el API. </br> Agregar user stories para la landing page y las technical stories, priorizar las user stories de acuerdo al core del negocio.<br><br/>Capitulo V: Se agregaron todos los componentes faltantes del sprint 2, componentes para el frontend de la aplicación, tales como login, register y store.<br/>Corrección del faq.<br/>Creación de la versión 1 del backend del sitio web utilizando C#. <br/><br/>Corrección de la parte frontend de la aplicación.<br/>Registro del Student Outcome y del cuadro de registro de versiones.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                         |

# Project Report Collaboration Insights

URL del repositorio para el Project Report: https://github.com/upc-coders/MapVet-Report

La creación del proyecto se llevó a cabo utilizando GitHub y se documentó en formato Markdown, siguiendo la metodología de GitFlow.

Network graph

<a href="https://ibb.co/Yh3TXPZ"><img src="https://i.ibb.co/HqVpCBd/network.png" alt="network" border="0"></a>

Conventional Commits

<a href="https://ibb.co/xzprWSX"><img src="https://i.ibb.co/zPytphQ/conventional.png" alt="conventional" border="0"></a>

Colaboradores del proyecto

<a href="https://ibb.co/ZfhbSpL"><img src="https://i.ibb.co/wdK27F0/colaboradores.png" alt="colaboradores" border="0"></a>

# Student Outcome

En Ingeniería de Software el logro de curso contribuye a alcanzar el:

**ABET - EAC - Student Outcome 5:**

**Criterio:** _La capacidad de funcionar efectivamente en un equipo cuyos miembros juntos proporcionan liderazgo, crean un entorno de colaboración e inclusivo, establecen objetivos, planifican tareas y cumplen objetivos._

En el siguiente cuadro se describe las acciones realizadas y enunciados de conclusiones por parte del grupo, que permiten sustentar el haber alcanzado el logro del ABET – EAC - Student Outcome 5.



| Criterio específico                                                                                                                                  | Acciones realizadas                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      | Conclusiones                                                                                                                                                                                                                                                                                                                      |
|------------------------------------------------------------------------------------------------------------------------------------------------------|------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Participa en equipos multidisciplinarios con eficacia, eficiencia y objetividad, en el marco de un proyecto en soluciones de ingeniería de software. | **TB1**<br>*Juan Calisaya*<br>Se desarrolo el analisis de las entrevistas que sirvieron para la creacion de arquetipos y aportar al Needflinding que influirá para User Personas y se implementó el index para el sprint 1.<br>**TP**<br>*Juan Calisaya*<br>Se corrigio los Web Applications UX/UI Design para los dueños de mascotas, se implemento el feature store al sprint 1 y se implemento el feat home al sprint 2.<br>**TB2**<br>*Juan Calisaya*<br>Para esta entrega, se hizo las preguntas y tres entrevistas de validacion, tambien, se implemento en el backend el endpoint de Faq<br>**TF**<br>*Juan Calisaya*<br>Cada miembro contribuyó al front-end y back-end para lograr el objetivo compartido de completar el backlog, combinando habilidades y finalizando componentes esenciales en ambas áreas del desarrollo.                                                                                                                                                                                                                                                                                          | La coordinación en equipo para esta entrega nos ayudó a completar distintos artefactos y también la identificación de los segmentos a los cuales va destinada la solución. Adicionalmente a ello, se trabajó en base a las cualidades de cada uno de los integrantes para enfocarnos en lo mejor que puede realizar cada miembro. |
|                                                                                                                                                      | **TB1**<br>Sebastian Ramirez<br>Se planificó y trato de ser lo más eficiente con la documentación, siempre con los valores y principios para implementar todo el proyecto <br> **TP**<br>Sebastian Ramirez<br>	Se precisó las habilidades de cada compañero para brindar un enfoque distinto para el desarrollo del proyecto, también se subsanó los puntos de mejora.  	     <br> **TB2**<br>  En busca de la mejora continua, se planteó un marco de trabajo amigable entre los compañeros del equipo y se planteó las fortalezas de cada uno de nosotros.                                                                                                                                                                                                                                                                                                                             | 	                                                                                                                                                                                                                                                                                                                                 |
|                                                                                                                                                      | **TB1**<br>*Daniel Valverde*<br>Grupalmente dividimos tareas entre los miembros para conocer a nuestros segmentos y a partir de ello, construir y prototipar productos como la landing page<br>	**TP**<br>Daniel Valverde<br>El equipo aplicó la mejora continua para superar las dificultades de la primera entrega, se concluyó con la implementación de las secciones faltantes. Además se trabajo en la implementación de la aplicación web en fase inicial	<br>**TB2** <br>*Daniel Valverde*<br>Para esta entrega, desarrollé los servicios del backend. <br>**TF** <br>*Daniel Valverde*<br>En el proyecto final, cada miembro contribuyó al desarrollo del front end y back end para cumplir con el objetivo compartido de completar el backlog. Se complementaron habilidades: se concluyeron componentes del frontend, los demás los hicieron funcionales los servicios backend | 	                                                                                                                                                                                                                                                                                                                                 |
|                                                                                                                                                      | **TB1**<br>*Juliana Yauricasa*<br>En esta entrega, se enfocó en definir el problema, comprender a fondo a los usuarios, crear historias de usuario sólidas y crear la landing Page de MapVet. <br>**TP** <br>*Juliana Yauricasa*<br>Para esta entrega, se corrigió el avance entregado y se creó la página web. Todo ello, en colaboracion de todo el equipo.	 <br>**TB2** <br>*Juliana Yauricasa*<br>Para esta entrega, se implemento mas componentes en el fronted y se entrego una primera entrega del backend. Ademas de realizar as heurísticas para la pagina web.                                                                                                                                                                                                                                                                                                                 | 	                                                                                                                                                                                                                                                                                                                                 |
| Conoce al menos un sector empresarial o dominio de aplicación de soluciones de software.                                                             | **TB1**<br>*Juan Calisaya*<br>Se investigo acerca de las plataformas de gestión de citas en las veterinarias.<br>**TP**<br>*Juan Calisaya*<br>Se busco informacion acerca de las tiendas online para implmentarlo en nuestro proyecto. <br>**TB2**<br>*Juan Calisaya*<br> Se investigó las preguntas para la entrevista de validación y sobre los endpoints para el backend<br>**TF**<br>*Juan Calisaya*<br>El producto final desarrollado satisface los requisitos de los segmentos objetivo, focalizándose especialmente en el sector de la salud animal al que estaba destinado.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              | Los artefactos diseñados nos permitieron conocer el sector empresarial al cual estamos enfocados. Adicionalmente a ello, investigar sobre las arquitecturas de software emergentes, nos abre la mente a soluciones y posibilidades de implementarlas en el proyecto en un futuro.                                                 |
|                                                                                                                                                      | **TB1**<br>Sebastian Ramirez<br>Se precisó la solución a un frente social para la mejora continúa del planeta y los seres que lo rodean.  <br>**TP**<br>Sebastian Ramirez<br> Se remplanteó la idea y los objetivos para el proyecto con el desarrollo de cada una de las partes de los miembros del proyecto. <br>  **TB2**<br>Sebastian Ramirez<br>  En base a lo desarrollado en el sprint anterior, se enfocó en las fortalezas de cada uno y se planteo en investigar una solución para cada uno.<br> **TF1**<br>  Se trabajó de acuerdo a los requisitos del proyecto y también realizando análisis de los usuarios que participarán en la aplicación.                                                                                                                                                                                                                                                                                                                                                                                    | 	                                                                                                                                                                                                                                                                                                                                 | 	                                                                                                                                                                         |
|                                                                                                                                                      | **TB1**<br>*Daniel Valverde*<br> Se desarrollaron diversos artefactos para descubrir las necesidades de los segmentos objetivo<br>	**TP**<br>Daniel Valverde<br> Se trabajó en la creacion de la aplicación web para el sector veterinarias en fase inicial implementado una de sus necesidades principales. <br>**TB2** <br>*Daniel Valverde*<br> Se desarrollaron los servicios del backend necesarios para presentar una aplicación funcional, de momento con funcionalidades que cubren a la competencia <br>**TF** <br>*Daniel Valverde* <br>El producto final que se logró desarrollar presenta las características necesarias para cubrir las necesidades de los segmentos objetivo a los cuales iba dirigida la solución                                                                                                                                                         | 	                                                                                                                                                                                                                                                                                                                                 |
|                                                                                                                                                      | **TB1**<br>*Juliana Yauricasa*<br>Para el trabajo se realizo una investigación previa al desarrolllo de un sistema de gestión dedicado al cuidado de las mascotas. <br> **TP**<br>*Juliana Yauricasa*<br>Para este segundo entregable se creo la primera version de la página web, asimismo se corrigio las entrevistas.	<br>**TB2** <br>*Juliana Yauricasa*<br>Para esta entrega, se entrego la primera version del backend asimismo se realizo las pruebas correspondientes.                                                                                                                                                                                                                                                                                                                                                                                                           | 	                                                                            


## Capítulo I: Introducción

1.1. Startup Profile

1.1.1. [Descripción de la Startup](/Archivosmd/Chapter-I.md#111-descripción-de-la-startup)

1.1.2. [Perfiles de integrantes del equipo](/Archivosmd/Chapter-I.md#112-perfiles-de-integrantes-del-equipo)

1.2. Solution Profile

1.2.1 [Antecedentes y problemática](/Archivosmd/Chapter-I.md#121-antecedentes-y-problemática)

1.2.2 Lean UX Process

1.2.2.1. [Lean UX Problem Statements](/Archivosmd/Chapter-I.md#1221-lean-ux-problem-statements)

1.2.2.2. [Lean UX Assumptions](/Archivosmd/Chapter-I.md#1222-lean-ux-assumptions)

1.2.2.3. [Lean UX Hypothesis Statements](/Archivosmd/Chapter-I.md#1223-lean-ux-hypothesis-statements)

1.2.2.4. [Lean UX Canvas](/Archivosmd/Chapter-I.md#1224-lean-ux-canvas)

1.3. [Segmentos objetivo](/Archivosmd/Chapter-I.md#13-segmentos-objetivo)

## Capítulo II: Requirements Elicitation & Analysis

2.1. Competidores.

2.1.1. [Análisis competitivo](/Archivosmd/Chapter-II.md#211-análisis-competitivo)

2.1.2. [Estrategias y tácticas frente a competidores](/Archivosmd/Chapter-II.md#212-estrategias-y-tácticas-frente-a-competidores)

2.2. Entrevistas.

2.2.1. [Diseño de entrevistas](/Archivosmd/Chapter-II.md#221-diseño-de-entrevistas)

2.2.2. [Registro de entrevistas](/Archivosmd/Chapter-II.md#222-registro-de-entrevistas)

2.2.3. [Análisis de entrevistas](/Archivosmd/Chapter-II.md#223-análisis-de-entrevistas)

2.3. [Needfinding](/Archivosmd/Chapter-II.md#23-needfinding)

2.3.1. [User Personas](/Archivosmd/Chapter-II.md#231-user-personas)

2.3.2. [User Task Matrix](/Archivosmd/Chapter-II.md#232-user-task-matrix)

2.3.3. [User Journey Mapping](/Archivosmd/Chapter-II.md#233-user-journey-mapping)

2.3.4. [Empathy Mapping](/Archivosmd/Chapter-II.md#234-empathy-mapping)

2.3.5. [As-is Scenario Mapping](/Archivosmd/Chapter-II.md#235-as-is-scenario-mapping)

## Capítulo III: Requirements Specification

3.1. [To-Be Scenario Mapping](/Archivosmd/Chapter-III.md#31-to-be-scenario-mapping)

3.2. [User Stories](/Archivosmd/Chapter-III.md#32-user-stories)

3.3. [Impact Mapping](/Archivosmd/Chapter-III.md#33-impact-mapping)

3.4. [Product Backlog](/Archivosmd/Chapter-III.md#34-product-backlog)

## Capítulo IV: Product Design

4.1. [Style Guidelines](/Archivosmd/Chapter-IV.md#41-style-guidelines)

4.1.1. [General Style Guidelines](/Archivosmd/Chapter-IV.md#411general-style-guidelines)

4.1.2. [Web Style Guidelines](/Archivosmd/Chapter-IV.md#412web-style-guidelines)

4.2. [Information Architecture](/Archivosmd/Chapter-IV.md#42-information-architecture)

4.2.1. [Organization Systems](/Archivosmd/Chapter-IV.md#421organization-systems)

4.2.2. [Labeling Systems](/Archivosmd/Chapter-IV.md#422-labeling-systems)

4.2.3. [SEO Tags and Meta Tags](/Archivosmd/Chapter-IV.md#423-seo-tags-and-meta-tags)

4.2.4. [Searching Systems](/Archivosmd/Chapter-IV.md#424-searching-systems)

4.2.5. [Navigation Systems](/Archivosmd/Chapter-IV.md#425-navigation-systems)

4.3. Landing Page UI Design

4.3.1. [Landing Page Wireframe](/Archivosmd/Chapter-IV.md#431-landing-page-wireframe)

4.3.2. [Landing Page Mock-up](/Archivosmd/Chapter-IV.md#432-landing-page-mock-up)

4.4. [Web Applications UX/UI Design](/Archivosmd/Chapter-IV.md#44-web-applications-uxui-design)

4.4.1. [Web Applications Wireframes](/Archivosmd/Chapter-IV.md#441-web-applications-wireframes)

4.4.2. [Web Applications Wireflow Diagrams](/Archivosmd/Chapter-IV.md#442-web-applications-wireflow-diagrams)

4.4.2. [Web Applications Mock-ups](/Archivosmd/Chapter-IV.md#443-web-applications-mock-ups)

4.4.3. Web Applications User Flow Diagrams.

4.5. Web Applications Prototyping.

4.6. Domain-Driven Software Architecture.

4.6.1. [Software Architecture Context Diagram](/Archivosmd/Chapter-IV.md#461-software-architecture-context-diagram)

4.6.2. [Software Architecture Container Diagrams](/Archivosmd/Chapter-IV.md#462-software-architecture-container-diagram)

4.6.3. [Software Architecture Components Diagrams](/Archivosmd/Chapter-IV.md#463-software-architecture-components-diagram)

4.7. Software Object-Oriented Design.

4.7.1. [Class Diagrams](/Archivosmd/Chapter-IV.md#471-class-diagrams)

4.7.2. [Class Dictionary](/Archivosmd/Chapter-IV.md#472-class-dictionary)

4.8. [Database Design](/Archivosmd/Chapter-IV.md#48-database-design)

4.8.1. [Database Diagram](/Archivosmd/Chapter-IV.md#481-database-diagram)

## Capítulo V: Product Implementation, Validation & Deployment

5.1. [Software Configuration Management](/Archivosmd/Chapter-V.md#51-software-configuration-management)

5.1.1. [Software Development Environment Configuration](/Archivosmd/Chapter-V.md#511software-development-environment-configuration)

5.1.2. [Source Code Management](/Archivosmd/Chapter-V.md#512source-code-management)

5.1.3. [Source Code Style Guide & Conventions](/Archivosmd/Chapter-V.md#513source-code-style-guide--conventions)

5.1.4. [Software Deployment Configuration](/Archivosmd/Chapter-V.md#514software-deployment-configuration)

5.2. Landing Page, Services & Applications Implementation.

5.2.1. [Sprint 1](/Archivosmd/Chapter-V.md#521-sprint-1)

5.2.1.1. [Sprint Planning 1](/Archivosmd/Chapter-V.md#5211-sprint-planning-1)

5.2.1.2. [Sprint Backlog 1](/Archivosmd/Chapter-V.md#5212-sprint-backlog-1)

5.2.1.3. [Development Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5213-development-evidence-for-sprint-review)

5.2.1.4. [Testing Suite Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5214-execution-evidence-for-sprint-review)

5.2.1.5. [Execution Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5215--software-deployment-evidence-for-sprint-review)

5.2.1.6. [Services Documentation Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5216-team-collaboration-insights-during-sprint)

5.2.2. [Sprint 2](/Archivosmd/Chapter-V.md#522-sprint-2)

5.2.2.1. [Sprint Planning 2](/Archivosmd/Chapter-V.md#5221-sprint-planning-2)

5.2.2.1. [Sprint Backlog 2](/Archivosmd/Chapter-V.md#5222-sprint-backlog-2)

5.2.1.3. [Development Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5223-development-evidence-for-sprint-review)

5.2.1.4. [Testing Suite Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5214-execution-evidence-for-sprint-review)

5.2.1.5. [Execution Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5225-execution-evidence-for-sprint-review)

5.2.1.6. [Services Documentation Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5226-services-documentation-evidence-for-sprint-review)

5.2.1.7. [Software Deployment Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5227--software-deployment-evidence-for-sprint-review)

5.2.1.8. [Team Collaboration Insights during Sprint](/Archivosmd/Chapter-V.md#5228-team-collaboration-insights-during-sprint)

5.2.3. [Sprint 3](/Archivosmd/Chapter-V.md#523-sprint-3)  

5.2.3.1. [Sprint Planning 3](/Archivosmd/Chapter-V.md#5231-sprint-planning-3) 

5.2.3.2. [Sprint Backlog 3](/Archivosmd/Chapter-V.md#5222-sprint-backlog-3)

5.2.3.3. [Development Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5233-development-evidence-for-sprint-review)  

5.2.3.4. [Testing Suite Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5234-testing-suit-evidence-for-sprint-review)  

5.2.3.5. [Execution Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5235-execution-evidence-for-sprint-review)

5.2.1.6. [Services Documentation Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5216-services-documentation-evidence-for-sprint-review)

5.2.1.7. [Software Deployment Evidence for Sprint Review](/Archivosmd/Chapter-V.md#5217-software-deployment-evidence-for-sprint-review)

5.2.1.8. [Team Collaboration Insights during Sprint](/Archivosmd/Chapter-V.md#5218-team-collaboration-insights-during-sprint)

5.3. [Validation Interviews](/Archivosmd/Chapter-V.md#53-validation-interviews)

5.3.1. [Diseño de Entrevistas](/Archivosmd/Chapter-V.md#531-diseño-de-entrevistas)

5.3.2. [Registro de Entrevistas](/Archivosmd/Chapter-V.md#532-registro-de-entrevistas)

5.3.3. [Evaluaciones según heurísticas](/Archivosmd/Chapter-V.md#533-evaluaciones-según-heurísticas)

5.4. [Video About-the-Product](/Archivosmd/Chapter-V.md#54-video-about-the-product)



**Conclusiones**

**Conclusiones y recomendaciones**

**Video About-the-Team**

**Bibliografía**

**Anexos**
